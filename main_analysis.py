...
# ============================================================
NON_DRUG_VALUES = {
    "prescription from treatment plan", "prescription", "prescribed",
    "from treatment plan", "treatment plan", "see treatment plan",
    "as prescribed", "as directed", "refer to pharmacy",
...
def chart_patient_demographics(patient):
    ...
    if "Is Admitted" in patient.columns and patient["Is Admitted"].notna().any():
        ...
        axes[1, 1].set_title("Admission Status"); axes[1, 1].set_ylabel("")
    else:
        axes[1, 1].axis("off")
    plt.tight_layout(); save_chart("patient_demographics", fig)
...
def chart_registration_trends(patient):
    ...
    p.groupby("Month").size().plot(kind="line", marker="o", color="navy", ax=ax)
    ax.set_title("Monthly Patient Registrations"); ax.set_ylabel("New Patients")
    plt.tight_layout(); save_chart("monthly_registrations", fig)

    p["DayOfWeek"] = p["Created At"].dt.day_name()
    order = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]
    fig, ax = plt.subplots(figsize=(10, 5))
    sns.countplot(data=p, x="DayOfWeek", order=order, palette="viridis", ax=ax)
    ax.set_title("Registrations by Day of Week"); plt.xticks(rotation=45)
    plt.tight_layout(); save_chart("registrations_by_dayofweek", fig)
...
def chart_vitals(nursing):
    ...
    for ax, col in zip(axes.flatten(), vitals):
        sns.boxplot(y=nursing[col].dropna(), ax=ax, color="lightblue"); ax.set_title(col)
    for ax in axes.flatten()[len(vitals):]:
        ax.axis("off")
    plt.tight_layout(); save_chart("vitals_boxplots", fig)
...
def chart_consultations(consultations, patient):
    ...
    if referrals:
        ...
        ref_counts = consultations[referrals].apply(pos)
    ...
    if "Created At" in consultations.columns and consultations["Created At"].notna().any():
        ...
        plt.tight_layout(); save_chart("monthly_consultations", fig)
        ...
        plt.tight_layout(); save_chart("consultations_by_hour", fig)
...
def chart_lab_tests(lab_tests):
    ...
    if lab_cols:
        ...
        plt.tight_layout(); save_chart("lab_test_results", fig)
    ...
    if {"Created At", "Completed At"}.issubset(lab_tests.columns):
        ...
        if not tat.empty:
            ...
            plt.tight_layout(); save_chart("lab_turnaround_time", fig)
...
def chart_optical(optical):
    ...
    if "Is Walk In" in optical.columns:
        ...
        ax.set_title("Walk-in vs Referred Optical Patients"); ax.set_ylabel("")
        plt.tight_layout(); save_chart("optical_walkin_vs_referred", fig)
    ...
    if "Glasses Allocated" in optical.columns:
        ...
        plt.tight_layout(); save_chart("glasses_allocated", fig)

    fig, axes = plt.subplots(1, 2, figsize=(12, 4))
    if "Visual Acuity Left" in optical.columns:
        sns.countplot(data=optical, x="Visual Acuity Left", palette="Blues", ax=axes[0])
        axes[0].set_title("Visual Acuity – Left"); axes[0].tick_params(axis="x", rotation=45)
    else:
        axes[0].axis("off")
    if "Visual Acuity Right" in optical.columns:
        sns.countplot(data=optical, x="Visual Acuity Right", palette="Greens", ax=axes[1])
        axes[1].set_title("Visual Acuity – Right"); axes[1].tick_params(axis="x", rotation=45)
    else:
        axes[1].axis("off")
    plt.tight_layout(); save_chart("visual_acuity", fig)
...
def chart_pharmacy(pharmacy, drugs):
    if pharmacy is not None and not pharmacy.empty and "Drug Name" in pharmacy.columns:
        ...
        if not top_drugs.empty:
            ...
            plt.tight_layout(); save_chart("top15_drugs", fig)
    ...
    if (pharmacy is not None and not pharmacy.empty
        ...
        ax.set_title("Pharmacy Dispense Rate"); ax.set_ylabel("")
        plt.tight_layout(); save_chart("dispense_rate", fig)
    ...
    if (drugs is not None and not drugs.empty
        ...
        ax.set_title("Drug Stock by Category"); plt.xticks(rotation=45)
        plt.tight_layout(); save_chart("stock_by_category", fig)
...
def chart_patient_journey(patient, consultations, lab_tests, pharmacy, optical):
    ...
    counts = [_n(patient, "Id", "Id"), _n(consultations), _n(lab_tests),
              _n(pharmacy), _n(optical)]
    fig, ax = plt.subplots(figsize=(10, 5))
    sns.barplot(x=stages, y=counts, palette="viridis", ax=ax)
    ax.set_title("Patient Care Pathway Funnel"); ax.set_ylabel("Unique Patients")
    for i, v in enumerate(counts):
        ax.text(i, v + 0.5, str(v), ha="center")
    plt.tight_layout(); save_chart("patient_journey_funnel", fig)
